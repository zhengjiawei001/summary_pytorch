...
def result_index2text(hyp, vocab, batch):
    hyp.article, hyp.real_abstract, x_len, y_len, article_oovs, len_oovs = batch
    # article_oovs = batch[0]["article_oovs"].numpy()[0]
...
def beam_decode(model, batch, vocab, params, print_info=False):
    ...
    def decoder_one_step(encoder_output, decoder_input, hn, cn, enc_input,
                         batch_oov_length, encoder_pad_mask,
                         use_coverage, coverage_vector):
        # 单个时间步，运行
        context_vector, attention_weights, coverage_vector1 = model.attention(hn,
                                                                              encoder_output,
                                                                              encoder_pad_mask,
                                                                              use_coverage=use_coverage,
                                                                              prev_coverage=coverage_vector)

        p_vocab, (hn, cn), p_gens = model.decoder(decoder_input.unsqueeze(1),
                                                  (hn, cn),
                                                  context_vector)
        final_dist = model.get_final_distribution(enc_input,
                                                  p_gens,
    ...
    # article_oovs = oov
    enc_output, (hn, cn) = model.encoder(enc_input)
    # 初始化batch size个 假设对象
    ...
    hyps = [Hypothesis(tokens=[start_index],
                       log_probs=[0.0],
                       hn=hn[0].unsqueeze(0).transpose(0, 1),
                       cn=cn[0].unsqueeze(0).transpose(0, 1),
                       attn_dists=[],
                       p_gens=[],
    ...
    while steps < params['max_dec_len'] and len(results) < params['beam_size']:
        ...
        if steps == 0:
            hn = torch.cat([h.hn for h in hyps], dim=1)
            cn = torch.cat([h.cn for h in hyps], dim=1)
            prev_coverage = torch.tensor(coverage_vector).squeeze(dim=-1)
        else:
            prev_coverage = torch.cat(coverage_vector)

            hn = torch.cat([h.hn for h in hyps], dim=0).transpose(0, 1)
            cn = torch.cat([h.cn for h in hyps], dim=0).transpose(0, 1)
        ...
        hn = decoder_results['hn'].transpose(0, 1)
        cn = decoder_results['cn'].transpose(0, 1)
        ...
        for i in range(num_orig_hyps):
            ...
            h, hn_new, cn_new, attn_dist = hyps[i], hn[i], cn[i], attention_weights[i]
            ...
            for j in range(params['beam_size'] * 2):
                # 构造可能的情况
                new_hyp = h.extend(token=top_k_ids[i, j].item(),
                                   log_prob=top_k_log_probs[i, j],
                                   hn=hn_new.unsqueeze(0),
                                   cn=cn_new.unsqueeze(0),
                                   attn_dist=attn_dist,
                                   p_gen=p_gen,
                                   coverage=new_coverage_i.unsqueeze(0))
                # 添加可能情况
...
class Hypothesis:
    ...
    def __init__(self, tokens, log_probs, hn, cn, attn_dists, p_gens, coverage):
        ...
        self.hn = hn  # decoder hidden state after the last token decoding
        self.cn = cn
    ...
    def extend(self, token, log_prob, hn, cn, attn_dist, p_gen, coverage):
