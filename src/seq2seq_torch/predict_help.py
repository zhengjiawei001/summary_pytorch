# -*- coding: utf-8 -*-
# @Time    : 2021/7/26 4:00 下午
# @Author  : zhengjiawei
# @FileName: predict_help.py
# @Software: PyCharm
import math
...
def greedy_decode(model, data_x, batch_size, vocab, params):
    ...
    for i in tqdm(range(steps_epoch)):
        ...
        results += batch_greedy_decoder(model, batch_data, vocab, params)
    return results


def batch_greedy_decoder(model, batch_data, vocab, params):
    # 判断输入长度
    batch_size = len(batch_data)
    # 开辟结果存储list
    predictions_ = [''] * batch_size

    inputs = torch.Tensor(batch_data).long().to(params['device'])
    # 0. 初始化隐藏层输入
    hidden = torch.zeros((2, batch_size, params['enc_units'])).to(params['device'])
    # 1. 构建encoder
    # print('hidden.shape:', hidden.shape)  # 2,7,256
    enc_output, enc_hidden = model.encoder(inputs, hidden)
    # enc_output (batch size, sequence length, units)
    # enc_hidden (batch size, units)
    # 2. 复制
    dec_hidden = enc_hidden  # (32,112)
    # print('dec_hidden.shape', dec_hidden.shape)
    # 3. <START> * BATCH_SIZE

    dec_input = torch.from_numpy(np.expand_dims([vocab.word_to_id(vocab.START_DECODING_INDEX)] * batch_size, axis=1))
    dec_input = dec_input.to(params['device'])
    # print('dec_hidden.shape:', dec_hidden.shape)
    for t in range(params['max_dec_len']):
        # 单步预测
        # dec_hidden = dec_hidden.to(params['device'])
        # enc_output = enc_output.to(params['device'])
        # print(dec_input.size()) torch.Size([32, 1]) (batch_size, 1)
        # print(dec_hidden.size())  (batch size, 2*units)
        # print(enc_output.size())  (batch size, sequence length, 2*units)
        ...
        # id转换 贪婪搜索
        dec_hidden = dec_hidden.permute(1, 0, 2).reshape(batch_size, -1)

        predicted_ids = torch.argmax(predictions.cpu(), dim=1).numpy()

        for index, predicted_id in enumerate(predicted_ids):
            predictions_[index] += vocab.id_to_word(predicted_id) + ' '

        # 上一步结果用于下一步预测
        dec_input = torch.unsqueeze(torch.tensor(predicted_ids), 1).to(params['device'])
    results = []
    for predict in predictions_:
        # 去掉句子前后空格
...
def beam_decode(model, batch, vocab, params):
    ...
    def decoder_one_step(enc_output, dec_input, dec_hidden):
        ...
        # 拿到top k个index 和 概率
        top_k_probs, top_k_ids = torch.topk(torch.squeeze(final_pred), k=params["beam_size"] * 2)
        # 计算log概率
    ...
    while steps < params['max_dec_len'] and len(results) < params['beam_size']:
        ...
        latest_tokens = [h.latest_token if isinstance(h.latest_token, int) else h.latest_token.item() for h in hyps]
        ...
        # dec_input = torch.unsqueeze(torch.tensor(latest_tokens),1)
        dec_input = torch.from_numpy(np.expand_dims(latest_tokens, axis=1))
        # print('latest_tokens1111:', latest_tokens)
        dec_input = dec_input.to(params['device'])

        dec_hidden = torch.stack(hiddens, dim=0).cuda()
        # print('dec_hidden.shape:', dec_hidden.shape)  # (batch_size, hidden_dim * 2)
        # 单步运行decoder 计算需要的值
        ...
        dec_hidden = decoder_results['dec_hidden'].reshape(decoder_results['dec_hidden'].size(1), -1)
        # print('dec_hidden.shape.....:',dec_hidden.shape)
        ...
        for i in range(num_orig_hyps):
            # print('dec_hidden[i].shape:',dec_hidden[i].shape)
            h, new_hidden, attn_dist = hyps[i], dec_hidden[i], attention_weights[i]
            # 分裂 添加 beam size 种可能性
            for j in range(params['beam_size'] * 2):
                # 构造可能的情况
                # print('top_k_ids[i, j].cpu().numpy():',top_k_ids[i, j].cpu().numpy())
                new_hyp = h.extend(token=top_k_ids[i, j].cpu().item(),
                                   log_prob=top_k_log_probs[i, j].cpu(),
                                   hidden=new_hidden.cpu(),
                                   attn_dist=attn_dist.cpu())
                # 添加可能情况
