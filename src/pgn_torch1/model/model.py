...
from src.pgn_torch1.utils import config
from src.pgn_torch1.utils.utils import replace_oovs
...
class Attention(nn.Module):
    ...
    def forward(self, decoder_states, encoder_output, x_padding_masks, coverage_vector):
        ...
        normalization_factor = attention_weights.sum(1,
                                                     keepdim=True)  # Normalize attention weights after excluding padded positions.
...
class PGN(nn.Module):
    ...
    def forward(self, x, x_len, y, len_oovs, batch, num_batches, teacher_forcing):
        ...
        for t in range(y.shape[1] - 1):
            ...
            y_t = y[:, t + 1]
            context_vector, attention_weights, coverage_vector = self.attention(decoder_states, encoder_output,
